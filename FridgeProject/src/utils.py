...
# Basic items list to check against
BASIC_ITEMS = [
    'apple', 'banana', 'milk', 'eggs', 'cheese',
    'bread', 'butter', 'carrot', 'tomato', 'potato'
]

# Estimated shelf life in days (just for recommendation logic)
SHELF_LIFE = {
    'milk': 7,
    'eggs': 21,
    'bread': 5,
...
def _calculate_recommendations(items_tuple, time_bucket):
    ...
    Accepts tuple of ItemData and time bucket.
    """
    scored_items = []
    now = datetime.utcnow()
...
def get_recommendations(items):
    ...
    # Time bucket: e.g. every hour (3600 seconds) to allow cache expiry/update
