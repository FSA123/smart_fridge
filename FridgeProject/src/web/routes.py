...
SAVE_PATH = "images"
CROPS_PATH = "src/web/static/crops" # Should match where detector saves
...
def api_data():
    ...
    # We want to return detailed items for the grid
    # and also aggregated recommendations/missing

    recommendations = get_recommendations(active_items)
    missing = get_missing_items(active_items)

    # Format detailed items for Grid View
    items_data = []
    ...
    # Format for JSON
    data = {
        'items': items_data, # List of individual items with crop_path
        'inventory': recommendations, # Legacy/Sorted list
        'missing': missing,
